...
const createBook=async function(req,res){
     try {
        ...
        if (!isValid(ISBN)) return res.status(400).send({ status: false, message: "phone no. is required" })
        if (!isValid(category)) return res.status(400).send({ status: false, message: "password is required" })
        if (!isValid(subcategory)) return res.status(400).send({ status: false, message: "password is required" })
        if (!isValid(releasedAt)) return res.status(400).send({ status: false, message: "password is required" })
      ...
      if(reviews){
       if(reviews!=0) return res.status(406).send({ status: false, message:'default value of reviews is 0 while book registering' })
      }
...
const getBooks = async function (req, res){
    try {
        ...
        const{userId, category, subcategory} = data
            ...
            if(userId){
        if(mongoose.isValidObjectId(userId)) {res.status(400).send({status: false, message:"please enter a correct userId"})}
            }
       const getBook={isDeleted:false}
       if(title) getBook["userId"]=userId
        if(category) getBook["category"]=category
        if(subcategory) getBook["subcategory"]=subcategory

            if(data.isDeleted == true) {res.status(404).send({status: false, message:"you can't find deleted books"})}

            
            const books = await bookModel.find(getBook).select({id:1, title:1, excerpt:1, userId:1, category:1, reviews:1, releaseAt:1})
            if(books.length == 0){res.status(404).send({status: false, message:"There is no Book "})}
        ...
        {res.status(500).send({status: false, msg:"please provide the valid information in query"})}
    }
}
...
const booksById = async function(req, res){
    try {
        ...
        if(id){
            if(mongoose.isValidObjectId(id)) {res.status(400).send({status: false, msg:"please enter a correct Id"})}
                }

        const findBook = await bookModel.findById(id)
        if(!findBook.id) {res.status(404).send({status: false, msg:"book's not found"})}
